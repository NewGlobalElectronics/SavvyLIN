...
bool CANConnection::sendFrame(const LINFrame& pFrame)
{
    /* make sure we execute in mThread context */
    if( mThread_p && (mThread_p != QThread::currentThread()) )
    {
...
bool CANConnection::sendFrames(const QList<LINFrame>& pFrames)
{
    /* make sure we execute in mThread context */
    if( mThread_p && (mThread_p != QThread::currentThread()) )
    {
        ...
        QMetaObject::invokeMethod(this, "sendFrames",
                                  Qt::BlockingQueuedConnection,
                                  Q_RETURN_ARG(bool, ret),
                                  Q_ARG(const QList<LINFrame>&, pFrames));
        return ret;
    }
...
LFQueue<LINFrame>& CANConnection::getQueue() {
    return mQueue;
}
...
void CANConnection::checkTargettedFrame(LINFrame &frame)
{
    ...
    foreach (const CANFltObserver filt, mBusData[frame.bus].mTargettedFrames)
    {
        ...
        if (maskedID == filt.id) {
            ...
            QMetaObject::invokeMethod(filt.observer, "gotTargettedFrame",Qt::QueuedConnection, Q_ARG(LINFrame, frame));
        }
    }
}

bool CANConnection::piSendFrames(const QList<LINFrame>& pFrames)
{
