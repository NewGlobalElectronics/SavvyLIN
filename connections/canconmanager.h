...
public:
     ...
     */
    bool sendFrame(const LINFrame& pFrame);

    //just the multi-frame version of above function.
    bool sendFrames(const QList<LINFrame>& pFrames);
    ...
    bool removeAllTargettedFrames(QObject *receiver);
     void injectTxFrameForDisplay(const LINFrame &frame);
signals:
    void framesReceived(CANConnection* pConn_p, QVector<LINFrame>& pFrames);
...
private:
    ...
    bool                   useSystemTime;
    QVector<LINFrame>      buslessFrames;
