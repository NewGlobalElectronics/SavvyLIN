...
FrameSenderObject::FrameSenderObject(const QVector<LINFrame> *frames)
{
...
void FrameSenderObject::processIncomingFrame(LINFrame *frame)
{
    for (int sd = 0; sd < sendingData.count(); sd++)
    {
        ...
        for (int trig = 0; trig < sendingData[sd].triggers.count(); trig++)
        {
            ...
            if (passedChecks && (thisTrigger->triggerMask & (TriggerMask::TRG_SIGNAL | TriggerMask::TRG_ID) ) )
            {
                ...
                db_MESSAGE *msg = dbHandler->findMessage(thisTrigger->ID);
                if (msg)
                {
                    DB_SIGNAL *sig = msg->sigHandler->findSignalByName(thisTrigger->sigName);
                    if (sig)
                    {
                        //first of all, is this signal really in this message we got?
                        if (sig->parentMessage && sig->parentMessage->ID == frame->frameId()) sigCheckPassed = true;
...
/// <returns></returns>
LINFrame* FrameSenderObject::lookupFrame(int ID, int bus)
{
