    ...
    ui(new Ui::dbSignalEditor)
{
            ...
            [=]()
            {
                ...
                case 2:
                    if (currentSignal->valType != SP_FLOAT)
                    {
                        ...
                        dbFile->setDirtyFlag();
                        if (dbMessage) //if we have a good msg reference we can use it to get the # of bytes expected.
                        {
    ...
    connect(ui->txtBitLength, &QLineEdit::textChanged,
            [=]()
            {
                ...
                if (dbMessage)
                {
                    if (temp > (int)(dbMessage->len * 8)) return;
                }
    ...
    connect(ui->txtMultiplexValues, &QLineEdit::editingFinished,
            [=]()
            {
                if (currentSignal == nullptr) return;
                int temp;
                if (currentSignal->multiplexdbString(DB_SIGNAL::MuxStringFormat_UI) != ui->txtMultiplexValues->text())
                {
                    //TODO: could look up the multiplexor and ensure that the value is within a range that the multiplexor could return
                    QString errorString;
                    if (!currentSignal->parsedbMultiplexUiString(ui->txtMultiplexValues->text(), errorString)) {
    ...
    connect(ui->rbExtended, &QRadioButton::toggled,
            [=](bool state)
            {
                ...
                if (!currentSignal->isMultiplexed || !currentSignal->isMultiplexor)
                {
                    ...
                    if (dbMessage->multiplexorSignal == currentSignal) dbMessage->multiplexorSignal = nullptr;
    ...
    connect(ui->rbMultiplexor, &QRadioButton::toggled,
            [=](bool state)
            {
                ...
                if (currentSignal->isMultiplexed || !currentSignal->isMultiplexor)
                {
                    ...
                    dbMessage->multiplexorSignal = currentSignal;
                    ui->txtMultiplexValues->setEnabled(currentSignal->isMultiplexed);
                    ui->cbMultiplexParent->setEnabled(currentSignal->isMultiplexed);
                    fillSignalForm(currentSignal);
                }
    ...
    connect(ui->cbMultiplexParent, &QComboBox::textActivated,
            [=]()
            {
                ...
                DB_SIGNAL *newSig = dbMessage->sigHandler->findSignalByName(ui->cbMultiplexParent->currentText());
                DB_SIGNAL *oldParent = currentSignal->multiplexParent;
...
void dbSignalEditor::fillSignalForm(DB_SIGNAL *sig)
{
    //sanity checks
    if (!dbMessage) return;
    if (!dbMessage->sigHandler) return;
    ...
    ui->txtMultiplexValues->setText(sig->multiplexdbString(DB_SIGNAL::MuxStringFormat_UI));
    ...
    int numSigs = dbMessage->sigHandler->getCount();
    for (int i = 0; i < numSigs; i++)
    {
        DB_SIGNAL *sig_iter = dbMessage->sigHandler->findSignalByIdx(i);
...
void dbSignalEditor::popFromUndoBuffer()
{
    if (undoBuffer.empty())
    {
        dbFile->clearDirtyFlag(); //TODO: Don't do this. Implement per-item dirty flags.
