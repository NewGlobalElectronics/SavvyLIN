...
void dbSignalHandler::removeAllSignals()
{
    sigs.clear();
}

int dbSignalHandler::getCount()
{
    return sigs.count();
}

void dbSignalHandler::sort()
{
...
db_MESSAGE* dbMessageHandler::findMsgByID(uint32_t id)
{
    ...
    for (int i = 0; i < messages.count(); i++)
    {
        ...
        else if (matchingCriteria == GMLAN_DB)
        {
...
{
    QList<db_MESSAGE*> messagesForNode;
...
bool dbMessageHandler::addMessage(db_MESSAGE &msg)
{
    messages.append(msg);
    return true;
}
...
void dbMessageHandler::removeAllMessages()
{
    messages.clear();
}

int dbMessageHandler::getCount()
{
...
bool dbMessageHandler::filterLabeling()
{
    return filterLabelingEnabled;
}

void dbMessageHandler::setFilterLabeling(bool filterLabeling)
{
    filterLabelingEnabled = filterLabeling;
}

MatchingCriteria_t dbMessageHandler::getMatchingCriteria()
{
    return matchingCriteria;
}

void dbMessageHandler::setMatchingCriteria(MatchingCriteria_t _matchingCriteria)
{
...
dbFile::dbFile(const dbFile& cpy) : QObject()
{
    ...
    db_nodes.clear();
    db_nodes.append(cpy.db_nodes);
    db_attributes.clear();
    db_attributes.append(cpy.db_attributes);
    isDirty = cpy.isDirty;
}
...
void dbFile::sort()
{
    std::sort(db_nodes.begin(), db_nodes.end()); //sort node names
...
{
    if (idx < 0) return nullptr;
    if (idx >= db_nodes.count()) return nullptr;
    return &db_nodes[idx];
}

db_NODE* dbFile::findNodeByName(QString name)
{
    if (db_nodes.length() == 0) return nullptr;
    for (int i = 0; i < db_nodes.length(); i++)
    {
...
db_NODE* dbFile::findNodeByNameAndComment(QString fullname)
{
    ...
    for (int i = 0; i < db_nodes.length(); i++)
    {
        if(db_nodes[i].comment.isEmpty())
            nameAndComment = db_nodes[i].name;
        else
            nameAndComment = db_nodes[i].name + " - " + db_nodes[i].comment;
...
QString dbFile::getFullFilename()
{
...
QString dbFile::getFilenameNoExt()
{
    return fileName.split(".db")[0];
}

QString dbFile::getPath()
{
    return filePath;
}

int dbFile::getAssocBus()
{
    return assocBuses;
}

void dbFile::setAssocBus(int bus)
{
    if (bus < -1) return;
...
{
    if (db_attributes.length() == 0) return nullptr;
    for (int i = 0; i < db_attributes.length(); i++)
    {
        if (db_attributes[i].name.compare(name, Qt::CaseInsensitive) == 0 && ((type == db_attributes[i].attrType) || (type == ATTR_TYPE_ANY) ) )
        {
...
{
    if (idx < 0) return nullptr;
    if (idx >= db_attributes.count()) return nullptr;
...
//It would be better if every node, message, and signal had a dirty flag. Then the dbFile getDirtyFlag
//function could traverse the tree and see if anything is dirty.
void dbFile::clearDirtyFlag()
{
    isDirty = false;
}

bool dbFile::getDirtyFlag()
{
    return isDirty;
}

db_MESSAGE* dbFile::parseMessageLine(QString line)
{
...
DB_SIGNAL* dbFile::parseSignalLine(QString line, db_MESSAGE *msg)
{
    QRegularExpression regex;
    QRegularExpressionMatch match;
...
bool dbFile::parseSignalMultiplexValueLine(QString line)
{
    QRegularExpression regex;
    QRegularExpressionMatch match;
    ...
    if (match.hasMatch())
    {
        db_MESSAGE *msg = messageHandler->findMsgByID(match.captured(1).toULong() & 0x1FFFFFFFUL);
        if (msg != nullptr)
        {
            DB_SIGNAL *thisSignal = msg->sigHandler->findSignalByName(match.captured(2));
            if (thisSignal != nullptr)
            {
                DB_SIGNAL *parentSignal = msg->sigHandler->findSignalByName(match.captured(3));
...
bool dbFile::parseSignalValueTypeLine(QString line)
{
    QRegularExpression regex;
    QRegularExpressionMatch match;
...
bool dbFile::parseValueLine(QString line)
{
    QRegularExpression regex;
    QRegularExpressionMatch match;
    ...
    if (match.hasMatch())
    {
        ...
        if (msg != nullptr)
        {
            DB_SIGNAL *sig = msg->sigHandler->findSignalByName(match.captured(2));
            if (sig != nullptr)
            {
                QString tokenString = match.captured(3);
                db_VAL_ENUM_ENTRY val;
...
bool dbFile::parseAttributeLine(QString line)
{
    QRegularExpression regex;
    QRegularExpressionMatch match;
    ...
    if (match.hasMatch())
    {
        ...
        db_ATTRIBUTE *foundAttr = findAttributeByName(match.captured(1));
        if (foundAttr)
        {
            qDebug() << "That message attribute does exist";
            db_MESSAGE *foundMsg = messageHandler->findMsgByID(match.captured(2).toULong() & 0x1FFFFFFFul);
            if (foundMsg)
            {
                qDebug() << "It references a valid, registered message";
                db_ATTRIBUTE_VALUE *foundAttrVal = foundMsg->findAttrValByName(match.captured(1));
                if (foundAttrVal) {
                    foundAttrVal->value = processAttributeVal(match.captured(3), foundAttr->valType);
                }
                else
                {
                    db_ATTRIBUTE_VALUE val;
    ...
    if (match.hasMatch())
    {
        ...
        if (foundAttr)
        {
            ...
            {
                qDebug() << "It references a valid, registered message";
                DB_SIGNAL *foundSig = foundMsg->sigHandler->findSignalByName(match.captured(3));
                if (foundSig)
                {
                    db_ATTRIBUTE_VALUE *foundAttrVal = foundSig->findAttrValByName(match.captured(1));
    ...
    if (match.hasMatch())
    {
        ...
        if (foundAttr)
        {
            ...
            db_NODE *foundNode = findNodeByName(match.captured(2));
            if (foundNode)
            {
                qDebug() << "References a valid node name";
                db_ATTRIBUTE_VALUE *foundAttrVal = foundNode->findAttrValByName(match.captured(1));
                if (foundAttrVal) foundAttrVal->value = processAttributeVal(match.captured(3), foundAttr->valType);
                else
                {
                    db_ATTRIBUTE_VALUE val;
...
bool dbFile::parseDefaultAttrLine(QString line)
{
    QRegularExpression regex;
    QRegularExpressionMatch match;
    ...
    if (match.hasMatch())
    {
        ...
        db_ATTRIBUTE *found = findAttributeByName(match.captured(1));
        if (found)
        {
...
bool dbFile::loadFile(QString fileName)
{
    QFile *inFile = new QFile(fileName);
    QString line, rawLine;
    QRegularExpression regex;
    QRegularExpressionMatch match;
    db_MESSAGE *currentMessage = nullptr;
    db_ATTRIBUTE attr;
    ...
    while (!inFile->atEnd()) {
        ...
        if (!inMultilineBU)
        {
            ...
            if (line.startsWith("CM_ BU_ "))
            {
                ...
                {
                    //qDebug() << "Comment was: " << match.captured(2);
                    db_NODE *node = findNodeByName(match.captured(1));
                    if (node != nullptr)
                    {
    ...
    db_ATTRIBUTE *mc_attr = findAttributeByName("matchingcriteria");
    if (mc_attr)
    {
    ...
    db_ATTRIBUTE *fl_attr = findAttributeByName("filterlabeling");
    if (fl_attr)
    {
    ...
    {
        db_MESSAGE *msg = messageHandler->findMsgByIdx(x);
        msg->bgColor = DefaultBG;
        msg->fgColor = DefaultFG;
        db_ATTRIBUTE_VALUE *thisBG = msg->findAttrValByName("GenMsgBackgroundColor");
        db_ATTRIBUTE_VALUE *thisFG = msg->findAttrValByName("GenMsgForegroundColor");
        ...
        {
            DB_SIGNAL *sig = msg->sigHandler->findSignalByIdx(y);
    ...
    {
        QMessageBox msgBox;
        QString msg = "db file loaded with errors!\n";
...
QVariant dbFile::processAttributeVal(QString input, db_ATTRIBUTE_VAL_TYPE typ)
{
    QVariant out;
    switch (typ)
    {
...
bool dbFile::parseAttribute(QString inpString, db_ATTRIBUTE &attr)
{
    bool goodAttr = false;
    QRegularExpression regex;
    QRegularExpressionMatch match;
...
bool dbFile::saveFile(QString fileName)
{
    ...
    for (int x = 0; x < db_nodes.count(); x++)
    {
        db_NODE node = db_nodes[x];
        if (node.name.compare("Vector__XXX", Qt::CaseInsensitive) != 0)
        {
            ...
            {
                foreach (db_ATTRIBUTE_VALUE val, node.attributes) {
    ...
    for (int x = 0; x < messageHandler->getCount(); x++)
    {
        ...
        {
            foreach (db_ATTRIBUTE_VALUE val, msg->attributes) {
        ...
        {
            DB_SIGNAL *sig = msg->sigHandler->findSignalByIdx(s);
            ...
            {
                foreach (db_ATTRIBUTE_VALUE val, sig->attributes) {
            ...
            if (sig->valList.count() > 0)
            {
                ...
                {
                    db_VAL_ENUM_ENTRY val = sig->valList[v];
    ...
    for (int x = 0; x < db_attributes.count(); x++)
    {
        msgOutput.append("BA_DEF_ ");
        switch (db_attributes[x].attrType)
        {
        ...
        msgOutput.append("\"" + db_attributes[x].name + "\" ");

        switch (db_attributes[x].valType)
        {
        case ATTR_INT:
            msgOutput.append("INT " + QString::number(db_attributes[x].lower) + " " + QString::number(db_attributes[x].upper));
            break;
        case ATTR_FLOAT:
            msgOutput.append("FLOAT " + QString::number(db_attributes[x].lower) + " " + QString::number(db_attributes[x].upper));
            break;
        case ATTR_STRING:
        ...
        case ATTR_ENUM:
            ...
            foreach (QString str, db_attributes[x].enumVals)
            {
        ...
        if (db_attributes[x].defaultValue.isValid())
        {
            defaultsOutput.append("BA_DEF_DEF_ \"" + db_attributes[x].name + "\" ");
            switch (db_attributes[x].valType)
            {
            case ATTR_STRING:
                defaultsOutput.append("\"" + db_attributes[x].defaultValue.toString() + "\";\n");
                break;
            case ATTR_ENUM:
                defaultsOutput.append("\"" + db_attributes[x].enumVals[db_attributes[x].defaultValue.toInt()] + "\";\n");
                break;
            case ATTR_INT:
                defaultsOutput.append(QString::number(db_attributes[x].defaultValue.toLongLong()) + ";\n");
                break;
            case ATTR_FLOAT:
                defaultsOutput.append(db_attributes[x].defaultValue.toString() + ";\n");
                break;
        ...
        for (int x = 0; x < messageHandler->getCount(); x++)
        {
            ...
            for (int s = 0; s < msg->sigHandler->getCount(); s++)
            {
                ...
                if (sig->isMultiplexed)
                {
                    ...
                    msgOutput.append(sig->multiplexdbString() + ";");
                    msgOutput.append("\n");
...
void dbHandler::savedbFile(int idx)
{
    ...
    QStringList filters;
    filters.append(QString(tr("db File (*.db)")));
    ...
    if (dialog.exec() == QDialog::Accepted)
    {
        ...
        if (!filename.contains('.')) filename += ".db";
...
//the user wants to replace an already loaded db.
...
dbFile* dbHandler::loadSecretCSVFile(QString filename)
{
    dbFile *thisFile;
    db_MESSAGE *pMsg;
    DB_SIGNAL *pSig;
...
dbFile* dbHandler::loadJSONFile(QString filename)
{
     QSettings settings;
     dbFile *thisFile;
     db_MESSAGE *pMsg;
         ...
         for (iter = jsonMessages.begin(); iter != jsonMessages.end(); iter++)
         {
             ...
             for (sigIter = jsonSigs.begin(); sigIter != jsonSigs.end(); sigIter++)
             {
                ...
                {
                    sig.valType = db_SIG_VAL_TYPE::UNSIGNED_INT;
                }
                else sig.valType = db_SIG_VAL_TYPE::SIGNED_INT;

                pMsg->sigHandler->addSignal(sig);
                if (sig.isMultiplexor) //if this signal was the multiplexor then store that info
                {
                    DB_SIGNAL *pSig = pMsg->sigHandler->findSignalByName(sig.name);
         ...
         {
             db_MESSAGE *msg = thisFile->messageHandler->findMsgByIdx(x);
             for (int y = 0; y < msg->sigHandler->getCount(); y++)
             {
                 DB_SIGNAL *sig = msg->sigHandler->findSignalByIdx(y);
                 //if this doesn't have a multiplex parent set but is multiplexed then it must have used
 ...
 * You give it a canbus frame and it'll tell you whether there is a loaded db file that can
 * interpret that frame for you.
 * Returns nullptr if there is no message definition that matches.
*/
db_MESSAGE* dbHandler::findMessage(const CANFrame &frame)
{
    for(int i = 0; i < loadedFiles.count(); i++)
    {
        if (loadedFiles[i].getAssocBus() == -1 || frame.bus == loadedFiles[i].getAssocBus())
        {
            db_MESSAGE* msg = loadedFiles[i].messageHandler->findMsgByID(frame.frameId());
...
// This function won't care which bus the db file is associated, but will return any message as long as ID matches and the file
...
db_MESSAGE* dbHandler::findMessageForFilter(uint32_t id, MatchingCriteria_t * matchingCriteria)
{
    for(int i = 0; i < loadedFiles.count(); i++)
    {
 ...
 * This has pitfalls because the same message name can easily exist in multiple db files AND nodes in a single file
 * By db standards only the MSG ID is required to be unique
*/
...
db_MESSAGE* dbHandler::findMessage(const QString msgName, const QString nodeName, const QString fileNameNoExt)
{
    db_MESSAGE *msg = nullptr;
    for(int i = 0; i < loadedFiles.count(); i++)
    {
...
db_MESSAGE* dbHandler::findMessage(const QString msgName, const QString fullyQualifiedNodeName)
{
...
dbHandler::dbHandler()
{
    // Load previously saved db file settings
    QSettings settings;
    qDebug() <<"Settings file: " << settings.fileName();
    int filecount = settings.value("db/FileCount", 0).toInt();
    qDebug() << "Previously loaded db file count: " << filecount;
    for (int i=0; i<filecount; i++)
    {
        QString filename = settings.value("db/Filename_" + QString::number(i),"").toString();
        dbFile * file = loaddbFile(filename);
        if (file)
        {
            int bus = settings.value("db/AssocBus_" + QString::number(i),0).toInt();
            file->setAssocBus(bus);

            MatchingCriteria_t matchingCriteria = (MatchingCriteria_t)settings.value("db/MatchingCriteria_" + QString::number(i),0).toInt();
            ...
            int filterLabeling = settings.value("db/FilterLabeling_" + QString::number(i),0).toInt();
            attr.attrType = ATTR_TYPE_MESSAGE;
            ...
            qInfo() << "Loaded db file" << filename << " (bus:" << bus
