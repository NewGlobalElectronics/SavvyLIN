...
void DB_SIGNAL::addMultiplexRange(int min, int max)
{
    if (min != max)
...
bool DB_SIGNAL::isSignalInMessage(const CANFrame &frame)
{
...
bool DB_SIGNAL::isValueMatchingMultiplex(int val) const
{
...
QString DB_SIGNAL::multiplexdbString(dbMuxStringFormat fmt) const
{
    QString ret;
    for (auto limitPair : multiplexLowAndHighValues) {
        if (fmt == MuxStringFormat_dbFile) {
            if (!ret.isEmpty())
...
void DB_SIGNAL::copyMultiplexValuesFromSignal(const DB_SIGNAL &signal)
{
...
/*
 * multiplexLowAndHighValues is private but the db saving code needs to be able to save a reasonable value for the multiplex value
 * for non-extended multiplex values. So, we grab the first element of the first item which should be the non-extended value
 */
int DB_SIGNAL::getSimpleMultiplexValue()
{
    return multiplexLowAndHighValues.at(0).first;
}

/**
 * @brief DB_SIGNAL::parsedbMultiplexUiString
 ...
 */
bool DB_SIGNAL::parsedbMultiplexUiString(const QString &multiplexes, QString &errorString)
{
...
bool DB_SIGNAL::multiplexesIdenticalToSignal(DB_SIGNAL *other) const
{
...
QString DB_SIGNAL::processSignalTree(const CANFrame &frame)
{
    QString build;
    int val;
    ...
    foreach (DB_SIGNAL *sig, multiplexedChildren)
    {
...
*/
bool DB_SIGNAL::processAsText(const CANFrame &frame, QString &outString, bool outputName, bool outputUnit)
{
    int64_t result = 0;
    bool isSigned = false;
...
bool DB_SIGNAL::processAsInt(const CANFrame &frame, int32_t &outValue)
{
    int32_t result = 0;
    bool isSigned = false;
...
bool DB_SIGNAL::processAsDouble(const CANFrame &frame, double &outValue)
{
    int64_t result = 0;
    bool isSigned = false;
    double endResult;
