...
dbLoadSaveWindow::dbLoadSaveWindow(const QVector<CANFrame> *frames, QWidget *parent) :
    QDialog(parent),
...
void dbLoadSaveWindow::loadFile()
{
    ...
    if (dialog.exec() == QDialog::Accepted)
    {
        ...
        if (dialog.selectedNameFilter() == filters[1])
        {
            ...
            file = dbHandler->loadJSONFile(filename);
        }
        if (dialog.selectedNameFilter() == filters[2])
        {
            if (!filename.contains('.')) filename += ".csv";
            file = dbHandler->loadSecretCSVFile(filename);
        }
    }
