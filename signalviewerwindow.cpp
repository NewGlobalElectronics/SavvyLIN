...
SignalViewerWindowLIN::SignalViewerWindowLIN(const QVector<LINFrame> *frames, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SignalViewerWindowLIN)
{
    ui->setupUi(this);
...
SignalViewerWindowLIN::~SignalViewerWindowLIN()
{
    delete ui;
}

void SignalViewerWindowLIN::updatedFrames(int numFrames)
{
    LINFrame thisFrame;
...
void SignalViewerWindowLIN::processFrame(LINFrame &frame)
{
    QString sigString;
    DB_SIGNAL *sig;
...
void SignalViewerWindowLIN::removeSelectedSignal()
{
...
void SignalViewerWindowLIN::loadNodes()
{
    int numFiles;
    ui->cbNodes->clear();
    if (dbHandler == nullptr) return;
    if ((numFiles = dbHandler->getFileCount()) == 0) return;
    qDebug() << numFiles;
    for (int f = 0; f < numFiles; f++)
    {
        dbFile* thisFile = dbHandler->getFileByIdx(f);
...
void SignalViewerWindowLIN::loadMessages(int idx)
{
    int numFiles = 0;
    ui->cbMessages->clear();
    if (dbHandler == nullptr) return;
    ...
    for (int f = 0; f < numFiles; f++)
    {
        qDebug() << dbHandler->getFileByIdx(f)->messageHandler->getCount();

        for (int x = 0; x < dbHandler->getFileByIdx(f)->messageHandler->getCount(); x++)
        {
            QString fullyQualifiedNodeName = dbHandler->getFileByIdx(f)->getFilenameNoExt() + Utility::fullyQualifiedNameSeperator + dbHandler->getFileByIdx(f)->messageHandler->findMsgByIdx(x)->sender->name;
            if(fullyQualifiedNodeName == displayedNodeName)
                ui->cbMessages->addItem(dbHandler->getFileByIdx(f)->messageHandler->findMsgByIdx(x)->name);
        }
    }
}

void SignalViewerWindowLIN::loadSignals(int idx)
{
    Q_UNUSED(idx);
    ...
    db_MESSAGE *msg = dbHandler->findMessage(msgName, fullyQualifiedNodeName);
...
void SignalViewerWindowLIN::addSignal()
{
    ...
    msg = dbHandler->findMessage(msgName, fullyQualifiedNodeName);
...
void SignalViewerWindowLIN::addSignal(DB_SIGNAL *sig)
{
...
void SignalViewerWindowLIN::saveSignalsFile()
{
    saveDefinitions();
}

void SignalViewerWindowLIN::loadSignalsFile()
{
    loadDefinitions(false);
}

void SignalViewerWindowLIN::appendSignalsFile()
{
    loadDefinitions(true);
}

void SignalViewerWindowLIN::clearSignalsTable()
{
    clearSignalsTable(true);
}

void SignalViewerWindowLIN::clearSignalsTable(bool askForConfirmation)
{
...
void SignalViewerWindowLIN::saveDefinitions()
{
    QString filename;
...
void SignalViewerWindowLIN::loadDefinitions(bool append)
{
    QString filename;
    QFileDialog dialog;
    QSettings settings;
        ...
        while (!inFile->atEnd()) {
            ...
            if (line.length() > 2)
            {
                ...
                if (tokens[0] == "SV1") //signal viewer save format v1
                {
                    ...
                    db_MESSAGE *msg;;
                    if( (msg = dbHandler->findMessage(msgId)) )
                    {
                        sig = msg->sigHandler->findSignalByName(sigName);
                        if(sig)
                            loadedSignals.append(sig);
                    }
                    else if ( (msg = dbHandler->findMessage(msgName)) )
                    {
