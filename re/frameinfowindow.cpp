...
const QColor FrameInfoWindowLIN::byteGraphColors[8] = {Qt::blue, Qt::green,  Qt::black, Qt::red, //0 1 2 3
                                                    Qt::gray, Qt::darkYellow, Qt::cyan,  Qt::darkMagenta}; //4 5 6 7
QPen FrameInfoWindowLIN::bytePens[8];

const int numIntervalHistBars = 20;

FrameInfoWindowLIN::FrameInfoWindowLIN(const QVector<LINFrame> *frames, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::FrameInfoWindowLIN)
{
    ui->setupUi(this);
    ...
    connect(ui->listFrameID, &QListWidget::currentTextChanged, 
        [this](QString itemText)
            {
            FrameInfoWindowLIN::updateDetailsWindow(FilterUtility::getId(itemText));
            } );

    connect(MainWindow::getReference(), &MainWindow::framesUpdated, this, &FrameInfoWindowLIN::updatedFrames);
    connect(ui->btnSave, &QAbstractButton::clicked, this, &FrameInfoWindowLIN::saveDetails);
...
void FrameInfoWindowLIN::setupByteGraph(QCustomPlot *plot, int num)
{
    ...
    connect(plot, &QCustomPlot::mouseDoubleClick, this, &FrameInfoWindowLIN::mouseDoubleClick);
}

void FrameInfoWindowLIN::mousePress()
{
...
void FrameInfoWindowLIN::mouseWheel()
{
    QCustomPlot *plot = qobject_cast<QCustomPlot *>(sender());
    if (!plot) return;
...
void FrameInfoWindowLIN::mouseDoubleClick()
{
    QCustomPlot *plot = qobject_cast<QCustomPlot *>(sender());
    bool hideMode = true;
...
void FrameInfoWindowLIN::showEvent(QShowEvent* event)
{
...
bool FrameInfoWindowLIN::eventFilter(QObject *obj, QEvent *event)
{
...
FrameInfoWindowLIN::~FrameInfoWindowLIN()
{
    removeEventFilter(this);
    delete ui;
}

void FrameInfoWindowLIN::closeEvent(QCloseEvent *event)
{
    Q_UNUSED(event)
    writeSettings();
    emit rejected();
...
void FrameInfoWindowLIN::readSettings()
{
    QSettings settings;
...
void FrameInfoWindowLIN::writeSettings()
{
    QSettings settings;
...
void FrameInfoWindowLIN::updatedFrames(int numFrames)
{
...
void FrameInfoWindowLIN::updateDetailsWindow(QString newID)
{
    int targettedID;
    ...
    if (targettedID > -1)
    {
        ...
        db_MESSAGE *msg = dbHandler->findMessageForFilter(targettedID, nullptr);
...
void FrameInfoWindowLIN::refreshIDList()
{
    int id;
    for (int i = 0; i < modelFrames->count(); i++)
    {
...
void FrameInfoWindowLIN::saveDetails()
{
    QString filename;
...
void FrameInfoWindowLIN::dumpNode(QTreeWidgetItem* item, QFile *file, int indent)
{
