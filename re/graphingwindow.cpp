...
GraphingWindowLIN::GraphingWindowLIN(const QVector<LINFrame> *frames, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::GraphingWindowLIN)
{
    ui->setupUi(this);
...
GraphingWindowLIN::~GraphingWindowLIN()
{
    delete ui;
}

void GraphingWindowLIN::showEvent(QShowEvent* event)
{
...
void GraphingWindowLIN::closeEvent(QCloseEvent *event)
{
    Q_UNUSED(event);
...
void GraphingWindowLIN::changeEvent(QEvent *event)
{
...
void GraphingWindowLIN::readSettings()
{
    QSettings settings;
...
void GraphingWindowLIN::writeSettings()
{
    QSettings settings;
...
void GraphingWindowLIN::updatedFrames(int numFrames)
{
    LINFrame thisFrame;
    QVector<double> x, y;
...
void GraphingWindowLIN::plottableClick(QCPAbstractPlottable* plottable, int dataIdx, QMouseEvent* event)
{
    Q_UNUSED(dataIdx);
...
void GraphingWindowLIN::plottableDoubleClick(QCPAbstractPlottable* plottable, int dataIdx, QMouseEvent* event)
{
    Q_UNUSED(dataIdx);
...
void GraphingWindowLIN::gotCenterTimeID(uint32_t ID, double timestamp)
{
    Q_UNUSED(ID)
...
void GraphingWindowLIN::titleDoubleClick(QMouseEvent* event, QCPTextElement* title)
{
  Q_UNUSED(event)
  Q_UNUSED(title)
...
void GraphingWindowLIN::axisDoubleClick(QCPAxis *axis, QCPAxis::SelectablePart part)
{
  qDebug() << "axisDoubleClick";
  if (part == QCPAxis::spAxisLabel) // Set an axis label by double clicking on it
  {
    bool ok;
    QString newLabel = QInputDialog::getText(this, "SavvyLIN Graphing", "New axis label:", QLineEdit::Normal, axis->label(), &ok);
    if (ok)
    {
...
void GraphingWindowLIN::legendSingleClick(QCPLegend *legend, QCPAbstractLegendItem *item)
{
...
void GraphingWindowLIN::legendDoubleClick(QCPLegend *legend, QCPAbstractLegendItem *item)
{
...
void GraphingWindowLIN::selectionChanged()
{
  /*
...
void GraphingWindowLIN::mousePress()
{
...
void GraphingWindowLIN::mouseWheel()
{
...
bool GraphingWindowLIN::eventFilter(QObject *obj, QEvent *event)
{
...
void GraphingWindowLIN::resetView()
{
...
void GraphingWindowLIN::zoomOut()
{
    QCPRange xrange = ui->graphingView->xAxis->range();
    QCPRange yrange = ui->graphingView->yAxis->range();
    if (ui->graphingView->xAxis->selectedParts().testFlag(QCPAxis::spAxis))
...
void GraphingWindowLIN::removeSelectedGraph()
{
...
void GraphingWindowLIN::editSelectedGraph()
{
    if (ui->graphingView->selectedGraphs().size() > 0)
    {
        int idx = -1;
...
void GraphingWindowLIN::removeAllGraphs()
{
...
void GraphingWindowLIN::rescaleAxis(QCPAxis *axis)
{
    axis->rescale(true);
}

void GraphingWindowLIN::rescaleToData()
{
...
void GraphingWindowLIN::toggleFollowMode()
{
    followGraphEnd = !followGraphEnd;
}

void GraphingWindowLIN::contextMenuRequest(QPoint pos)
{
...
void GraphingWindowLIN::saveGraphs()
{
...
void GraphingWindowLIN::saveSpreadsheet()
{
    QFileDialog dialog(this);
    QSettings settings;
    ...
    if (dialog.exec() == QDialog::Accepted)
    {
        ...
        */
        ...
        for (auto && graph : graphParams) {
            ...
            maxCount = std::max<int>(maxCount, graph.x.count());
        }
        qDebug() << "xMin: " << xMin;
        qDebug() << "xMax: " << xMax;
...
void GraphingWindowLIN::saveDefinitions()
{
    QFileDialog dialog(this);
    QSettings settings;
...
void GraphingWindowLIN::loadDefinitions()
{
    QFileDialog dialog;
    QSettings settings;
    ...
    if (dbHandler->getFileCount() == 0) dbHandler->createBlankFile();
        ...
        while (!inFile.atEnd()) {
            ...
            if (line.length() > 2)
            {
                ...
                if (tokens[0] == "Z") //very newest format, adds ability to set bus number
                {
                   ...
                   if (tokens.length() > 22)
                   {
                       db_MESSAGE *msg = dbHandler->findMessage(gp.ID);
                       if (msg)
                       {
                ...
                else //one of the two older formats then
                {
                    ...
                    if (tokens[1] == "S") //old signal based graph definition
                    {
                        ...
                        db_MESSAGE *msg = dbHandler->getFileByIdx(0)->messageHandler->findMsgByID(gp.ID);
                        if (msg != nullptr)
                        {
                            DB_SIGNAL *sig = msg->sigHandler->findSignalByName(tokens[2]);
                            if (sig)
                            {
                                gp.mask = 0xFFFFFFFF;
...
void GraphingWindowLIN::showParamsDialog(int idx = -1)
{
    dbHandler = dbHandler::getReference();

    NewGraphDialog *thisDialog = new NewGraphDialog(dbHandler, this);
...
void GraphingWindowLIN::addNewGraph()
{
    showParamsDialog(-1);
}

void GraphingWindowLIN::appendToGraph(GraphParams &params, LINFrame &frame, QVector<double> &x, QVector<double> &y)
{
    params.strideSoFar++;
    if (params.strideSoFar >= params.stride)
    {
        ...
        //there really is no way to set a graphable item as being stored as a float unless it was an actual db signal
        //So, if we have a db signal associated then use that, otherwise try to turn the above integer calculation into
...
void GraphingWindowLIN::createGraph(GraphParams &params, bool createGraphParam)
{
    int64_t tempVal; //64 bit temp value.
    double yminval=10000000.0, ymaxval = -1000000.0;
...
void GraphingWindowLIN::moveLegend()
{
    qDebug() << "moveLegend";
