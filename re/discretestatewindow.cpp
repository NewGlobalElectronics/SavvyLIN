...
DiscreteStateWindowLIN::DiscreteStateWindowLIN(const QVector<LINFrame> *frames, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::DiscreteStateWindowLIN)
{
    ui->setupUi(this);
...
DiscreteStateWindowLIN::~DiscreteStateWindowLIN()
{
...
bool DiscreteStateWindowLIN::eventFilter(QObject *obj, QEvent *event)
{
...
void DiscreteStateWindowLIN::typeChanged()
{
...
void DiscreteStateWindowLIN::updatedFrames(int numFrames)
{
    LINFrame thisFrame;
...
void DiscreteStateWindowLIN::refreshFilterList()
{
    int id;
...
void DiscreteStateWindowLIN::showEvent(QShowEvent* event)
{
...
void DiscreteStateWindowLIN::closeEvent(QCloseEvent *event)
{
    Q_UNUSED(event);
    writeSettings();
}

void DiscreteStateWindowLIN::readSettings()
{
    QSettings settings;
...
void DiscreteStateWindowLIN::writeSettings()
{
    QSettings settings;
...
void DiscreteStateWindowLIN::updateStateLabel()
{
    QPalette pal;
...
void DiscreteStateWindowLIN::handleStartButton()
{
    if (isRealtime)
    {
        ...
        {
            stateFrames.append(new QVector<LINFrame>());
        }
...
void DiscreteStateWindowLIN::handleTick()
{
    switch (operatingState)
    {
    case DWStates::IDLE:
...
void DiscreteStateWindowLIN::calculateResults()
{
    int minBits, maxBits;
    if (isRealtime)
    {
