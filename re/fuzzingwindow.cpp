...
FuzzingWindowLIN::FuzzingWindowLIN(const QVector<LINFrame> *frames, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::FuzzingWindowLIN)
{
    ui->setupUi(this);
    ...
    connect(ui->btnMarkAllHigh, &QPushButton::clicked, this, &FuzzingWindowLIN::markAllHigh);
    connect(ui->btnMarkAllLow, &QPushButton::clicked, this, &FuzzingWindowLIN::markAllLow);
    connect(ui->btnMarkAllAuto, &QPushButton::clicked, this, &FuzzingWindowLIN::markAllAuto);
    connect(ui->btnStartStop, &QPushButton::clicked, this, &FuzzingWindowLIN::toggleFuzzing);
    connect(ui->btnAllFilters, &QPushButton::clicked, this, &FuzzingWindowLIN::setAllFilters);
    connect(ui->btnNoFilters, &QPushButton::clicked, this, &FuzzingWindowLIN::clearAllFilters);
    connect(fuzzTimer, &QTimer::timeout, this, &FuzzingWindowLIN::timerTriggered);
    connect(ui->spinTiming, SIGNAL(valueChanged(int)), this, SLOT(changePlaybackSpeed(int)));
    connect(ui->listID, &QListWidget::itemChanged, this, &FuzzingWindowLIN::idListChanged);
...
FuzzingWindowLIN::~FuzzingWindowLIN()
{
    removeEventFilter(this);
    delete ui;
}

bool FuzzingWindowLIN::eventFilter(QObject *obj, QEvent *event)
{
...
void FuzzingWindowLIN::updatedFrames(int numFrames)
{
    int id;
...
void FuzzingWindowLIN::changePlaybackSpeed(int newSpeed)
{
    fuzzTimer->setInterval(newSpeed);
}

void FuzzingWindowLIN::changedDataByteText(int which, QString valu)
{
...
void FuzzingWindowLIN::changedNumDataBytes(int newVal)
{
...
void FuzzingWindowLIN::timerTriggered()
{
...
void FuzzingWindowLIN::clearAllFilters()
{
...
void FuzzingWindowLIN::setAllFilters()
{
    for (int i = 0; i < ui->listID->count(); i++)
    {
...
void FuzzingWindowLIN::calcNextID()
{
    if (seqIDScan)
    {
        if (rangeIDSelect)
...
void FuzzingWindowLIN::calcNextBitPattern()
{
    uint64_t accum;
...
void FuzzingWindowLIN::toggleFuzzing()
{
...
void FuzzingWindowLIN::refreshIDList()
{
    ui->listID->clear();
    foundIDs.clear();
...
void FuzzingWindowLIN::idListChanged(QListWidgetItem *item)
{
...
*/
void FuzzingWindowLIN::bitfieldClicked(int bitPos)
{
...
void FuzzingWindowLIN::redrawGrid()
{
...
void FuzzingWindowLIN::markAllHigh()
{
    for (int i = 0; i < 512; i++)
    {
        bitGrid[i] = 2;
...
void FuzzingWindowLIN::markAllLow()
{
    for (int i = 0; i < 512; i++)
    {
        bitGrid[i] = 0;
...
void FuzzingWindowLIN::markAllAuto()
{
    for (int i = 0; i < 512; i++)
    {
        bitGrid[i] = 1;
