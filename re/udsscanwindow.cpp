...
UDSScanWindowLIN::UDSScanWindowLIN(const QVector<LINFrame> *frames, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::UDSScanWindowLIN)
{
    ui->setupUi(this);
    ...
    connect(udsHandler, &UDS_HANDLER::newUDSMessage, this, &UDSScanWindowLIN::gotUDSReply);
    connect(ui->btnScanAll, &QPushButton::clicked, this, &UDSScanWindowLIN::scanAll);
    connect(ui->btnScanSelected, &QPushButton::clicked, this, &UDSScanWindowLIN::scanSelected);
    connect(waitTimer, &QTimer::timeout, this, &UDSScanWindowLIN::timeOut);
    connect(ui->btnSaveResults, &QPushButton::clicked, this, &UDSScanWindowLIN::saveResults);
    connect(ui->cbScanType, &QComboBox::currentTextChanged, this, &UDSScanWindowLIN::changedScanType);
    connect(ui->cbAllowAdaptiveOffset, &QCheckBox::toggled, this, &UDSScanWindowLIN::adaptiveToggled);
    ...
    connect(ui->btnAdd, &QPushButton::clicked, this, &UDSScanWindowLIN::addNewScan);
    connect(ui->btnDelete, &QPushButton::clicked, this, &UDSScanWindowLIN::deleteSelectedScan);
    connect(ui->btnSave, &QPushButton::clicked, this, &UDSScanWindowLIN::saveScans);
    connect(ui->btnLoad, &QPushButton::clicked, this, &UDSScanWindowLIN::loadScans);
    connect(ui->listScansToRun, &QListWidget::currentRowChanged, this, &UDSScanWindowLIN::displayScanEntry);
    connect(ui->ckShowNoReply, &QCheckBox::toggled, this, &UDSScanWindowLIN::setNoReplyVal);
    ...
    connect(ui->cbSessType, &QComboBox::currentTextChanged, this, &UDSScanWindowLIN::setSessType);
...
//not handling show no reply, max reply delay, reply offset, increment
    ...
    connect(ui->cbBuses, &QComboBox::currentTextChanged, this, &UDSScanWindowLIN::setBusToScan);
...
UDSScanWindowLIN::~UDSScanWindowLIN()
{
...
bool UDSScanWindowLIN::eventFilter(QObject *obj, QEvent *event)
{
...
void UDSScanWindowLIN::setControlState(QWidget & widget, bool valid)
{
...
void UDSScanWindowLIN::displayScanEntry(int idx)
{
...
void UDSScanWindowLIN::deleteSelectedScan()
{
...
void UDSScanWindowLIN::addNewScan()
{
    ScanEntry newEntry;
...
void UDSScanWindowLIN::loadScans()
{
    QString filename;
...
void UDSScanWindowLIN::saveScans()
{
    QString filename;
    QFileDialog dialog(qApp->activeWindow());
    QSettings settings;
...
QString UDSScanWindowLIN::generateListDesc(int idx)
{
    QString builder;
...
void UDSScanWindowLIN::adaptiveToggled()
{
    if (inhibitUpdates) return;
...
void UDSScanWindowLIN::numBytesChanged()
{
    if (inhibitUpdates) return;
...
void UDSScanWindowLIN::checkIDRange()
{
    if (inhibitUpdates) return;
...
void UDSScanWindowLIN::checkServiceRange()
{
    if (inhibitUpdates) return;
...
void UDSScanWindowLIN::checkSubFuncRange()
{
    if (inhibitUpdates) return;
...
void UDSScanWindowLIN::saveResults()
{
    QString filename;
...
void UDSScanWindowLIN::dumpNode(QTreeWidgetItem* item, QFile *file, int indent)
{
...
void UDSScanWindowLIN::sendOnBuses(UDS_MESSAGE test, int buses)
{
    test.bus = buses;
    sendingFrames.append(test);
}

void UDSScanWindowLIN::scanAll()
{
    sendingFrames.clear();
    for (int i = 0; i < scanEntries.count(); i++)
    {
...
void UDSScanWindowLIN::scanSelected()
{
    sendingFrames.clear();
    int idx = ui->listScansToRun->currentRow();
    if (idx < 0) return;
...
void UDSScanWindowLIN::startScan()
{
...
void UDSScanWindowLIN::stopScan()
{
    waitTimer->stop();
    sendingFrames.clear();
...
void UDSScanWindowLIN::setupScan(int idx)
{
    UDS_MESSAGE test;
...
void UDSScanWindowLIN::updatedFrames(int numFrames)
{
...
void UDSScanWindowLIN::gotUDSReply(UDS_MESSAGE msg)
{
    QString result;
...
void UDSScanWindowLIN::setupNodes(uint32_t replyID)
{
...
void UDSScanWindowLIN::sendNextMsg()
{
    currIdx++;
