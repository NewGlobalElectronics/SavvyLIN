...
const QColor FlowViewWindowLIN::graphColors[8] = {Qt::blue, Qt::green, Qt::black, Qt::red, //0 1 2 3
                                               Qt::gray, Qt::darkYellow, Qt::cyan, Qt::darkMagenta}; //4 5 6 7


FlowViewWindowLIN::FlowViewWindowLIN(const QVector<LINFrame> *frames, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::FlowViewWindowLIN)
{
    ui->setupUi(this);
    ...
    connect(ui->graphRangeSlider, &QSlider::valueChanged, this, &FlowViewWindowLIN::graphRangeChanged);
    connect(ui->check_0, &QCheckBox::stateChanged, this, &FlowViewWindowLIN::changeGraphVisibility);
    connect(ui->check_1, &QCheckBox::stateChanged, this, &FlowViewWindowLIN::changeGraphVisibility);
    connect(ui->check_2, &QCheckBox::stateChanged, this, &FlowViewWindowLIN::changeGraphVisibility);
    connect(ui->check_3, &QCheckBox::stateChanged, this, &FlowViewWindowLIN::changeGraphVisibility);
    connect(ui->check_4, &QCheckBox::stateChanged, this, &FlowViewWindowLIN::changeGraphVisibility);
    connect(ui->check_5, &QCheckBox::stateChanged, this, &FlowViewWindowLIN::changeGraphVisibility);
    connect(ui->check_6, &QCheckBox::stateChanged, this, &FlowViewWindowLIN::changeGraphVisibility);
    connect(ui->check_7, &QCheckBox::stateChanged, this, &FlowViewWindowLIN::changeGraphVisibility);

//    ui->timelineSlider->setTracking(true);
    connect(ui->timelineSlider, &QSlider::sliderPressed, this, &FlowViewWindowLIN::btnPauseClick);
    connect(ui->timelineSlider, &QSlider::valueChanged, this, &FlowViewWindowLIN::gotoFrame);
...
void FlowViewWindowLIN::showEvent(QShowEvent* event)
{
...
FlowViewWindowLIN::~FlowViewWindowLIN()
{
...
void FlowViewWindowLIN::closeEvent(QCloseEvent *event)
{
    Q_UNUSED(event);
    writeSettings();
...
void FlowViewWindowLIN::readSettings()
{
    QSettings settings;
...
void FlowViewWindowLIN::writeSettings()
{
    QSettings settings;
...
*/
bool FlowViewWindowLIN::eventFilter(QObject *obj, QEvent *event)
{
...
void FlowViewWindowLIN::changeGraphVisibility(int state){
...
void FlowViewWindowLIN::gotCellClick(int bitPosition)
{
...
void FlowViewWindowLIN::graphRangeChanged(int range) {
    ui->rangeValue->setText(QString::number(range));
    updateGraphLocation();
}

void FlowViewWindowLIN::updateTriggerValues()
{
...
void FlowViewWindowLIN::plottableDoubleClick(QCPAbstractPlottable* plottable, QMouseEvent* event)
{
    int id = 0;
...
void FlowViewWindowLIN::gotCenterTimeID(uint32_t ID, double timestamp)
{
    int64_t t_stamp;
...
void FlowViewWindowLIN::contextMenuRequestFlow(QPoint pos)
{
...
void FlowViewWindowLIN::contextMenuRequestGraph(QPoint pos)
{
  QMenu *menu = new QMenu(this);
  menu->setAttribute(Qt::WA_DeleteOnClose);
...
void FlowViewWindowLIN::saveFileGraph()
{
    QString filename;
...
void FlowViewWindowLIN::saveFileFlow()
{
    QString filename;
    QFileDialog dialog(this);
    QSettings settings;
...
void FlowViewWindowLIN::updatedFrames(int numFrames)
{
...
void FlowViewWindowLIN::removeAllGraphs()
{
  ui->graphView->clearGraphs();
  ui->graphView->replot();
}

void FlowViewWindowLIN::createGraph(int byteNum)
{
    int tempVal;
...
void FlowViewWindowLIN::refreshIDList()
{
    int id;
    for (int i = 0; i < modelFrames->count(); i++)
    {
        LINFrame thisFrame = modelFrames->at(i);
...
void FlowViewWindowLIN::changeID(QString newID)
{
    ...
    {
        LINFrame thisFrame = modelFrames->at(x);
...
void FlowViewWindowLIN::btnBackOneClick()
{
...
void FlowViewWindowLIN::btnPauseClick()
{
    ui->cbLiveMode->setChecked(false);
    playbackActive = false;
    playbackTimer->stop();
...
void FlowViewWindowLIN::btnReverseClick()
{
    ui->cbLiveMode->setChecked(false);
    playbackActive = true;
...
void FlowViewWindowLIN::btnStopClick()
{
    ui->cbLiveMode->setChecked(false);
    playbackTimer->stop(); //pushing this button halts automatic playback
    playbackActive = false;
...
void FlowViewWindowLIN::btnPlayClick()
{
    ui->cbLiveMode->setChecked(false);
    playbackActive = true;
    playbackForward = true;
...
void FlowViewWindowLIN::btnFwdOneClick()
{
    ui->cbLiveMode->setChecked(false);
    playbackTimer->stop();
    playbackActive = false;
...
void FlowViewWindowLIN::changePlaybackSpeed(int newSpeed)
{
    playbackTimer->setInterval(newSpeed);
}

void FlowViewWindowLIN::changeLooping(bool check)
{
    Q_UNUSED(check);
}

void FlowViewWindowLIN::timerTriggered()
{
...
void FlowViewWindowLIN::updateDataView()
{
...
void FlowViewWindowLIN::gotoFrame(int frame) {
...
void FlowViewWindowLIN::updatePosition(bool forward)
{
...
void FlowViewWindowLIN::updateGraphLocation()
{
    if (frameCache.count() == 0) return;
