...
ISOTP_InterpreterWindowLIN::ISOTP_InterpreterWindowLIN(const QVector<LINFrame> *frames, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ISOTP_InterpreterWindowLIN)
{
    ui->setupUi(this);
    ...
    connect(MainWindow::getReference(), &MainWindow::framesUpdated, this, &ISOTP_InterpreterWindowLIN::updatedFrames);
    connect(MainWindow::getReference(), &MainWindow::framesUpdated, decoder, &ISOTP_HANDLER::updatedFrames);
    connect(decoder, &ISOTP_HANDLER::newISOMessage, this, &ISOTP_InterpreterWindowLIN::newISOMessage);
    connect(udsDecoder, &UDS_HANDLER::newUDSMessage, this, &ISOTP_InterpreterWindowLIN::newUDSMessage);
    connect(ui->listFilter, &QListWidget::itemChanged, this, &ISOTP_InterpreterWindowLIN::listFilterItemChanged);
    connect(ui->btnAll, &QPushButton::clicked, this, &ISOTP_InterpreterWindowLIN::filterAll);
    connect(ui->btnNone, &QPushButton::clicked, this, &ISOTP_InterpreterWindowLIN::filterNone);
    connect(ui->btnCaptured, &QPushButton::clicked, this, &ISOTP_InterpreterWindowLIN::interpretCapturedFrames);

    connect(ui->tableIsoFrames, &QTableWidget::itemSelectionChanged, this, &ISOTP_InterpreterWindowLIN::showDetailView);
    connect(ui->btnClearList, &QPushButton::clicked, this, &ISOTP_InterpreterWindowLIN::clearList);
    connect(ui->btnSaveList, &QPushButton::clicked, this, &ISOTP_InterpreterWindowLIN::saveList);
...
ISOTP_InterpreterWindowLIN::~ISOTP_InterpreterWindowLIN()
{
    delete decoder;
    delete ui;
}

void ISOTP_InterpreterWindowLIN::showEvent(QShowEvent* event)
{
...
void ISOTP_InterpreterWindowLIN::closeEvent(QCloseEvent *event)
{
    Q_UNUSED(event)
...
bool ISOTP_InterpreterWindowLIN::eventFilter(QObject *obj, QEvent *event)
{
...
void ISOTP_InterpreterWindowLIN::readSettings()
{
    QSettings settings;
...
void ISOTP_InterpreterWindowLIN::writeSettings()
{
    QSettings settings;
...
void ISOTP_InterpreterWindowLIN::interpretCapturedFrames()
{
    clearList();
    decoder->updatedFrames(-2);
}

void ISOTP_InterpreterWindowLIN::listFilterItemChanged(QListWidgetItem *item)
{
    if (item)
    {
...
void ISOTP_InterpreterWindowLIN::filterAll()
{
...
void ISOTP_InterpreterWindowLIN::filterNone()
{
    for (int i = 0 ; i < ui->listFilter->count(); i++)
    {
...
void ISOTP_InterpreterWindowLIN::clearList()
{
 ...
 */
void ISOTP_InterpreterWindowLIN::saveList()
{
    QString buildString;
    QString filename;
...
void ISOTP_InterpreterWindowLIN::useExtendedAddressing(bool checked)
{
    decoder->setExtendedAddressing(checked);
    this->interpretCapturedFrames();
}

void ISOTP_InterpreterWindowLIN::updatedFrames(int numFrames)
{
...
void ISOTP_InterpreterWindowLIN::newUDSMessage(UDS_MESSAGE msg)
{
...
void ISOTP_InterpreterWindowLIN::newISOMessage(ISOTP_MESSAGE msg)
{
    int rowNum;
    QString tempString;
