...
FramePlaybackWindowLIN::FramePlaybackWindowLIN(const QVector<LINFrame> *frames, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::FramePlaybackWindowLIN)
{
    ui->setupUi(this);
    ...
    connect(ui->btnStepBack, &QAbstractButton::clicked, this, &FramePlaybackWindowLIN::btnBackOneClick);
    connect(ui->btnPause, &QAbstractButton::clicked, this, &FramePlaybackWindowLIN::btnPauseClick);
    connect(ui->btnPlayReverse, &QAbstractButton::clicked, this, &FramePlaybackWindowLIN::btnReverseClick);
    connect(ui->btnStop, &QAbstractButton::clicked, this, &FramePlaybackWindowLIN::btnStopClick);
    connect(ui->btnPlay, &QAbstractButton::clicked, this, &FramePlaybackWindowLIN::btnPlayClick);
    connect(ui->btnStepForward, &QAbstractButton::clicked, this, &FramePlaybackWindowLIN::btnFwdOneClick);
    connect(ui->btnSelectAll, &QAbstractButton::clicked, this, &FramePlaybackWindowLIN::btnSelectAllClick);
    connect(ui->btnSelectNone, &QAbstractButton::clicked, this, &FramePlaybackWindowLIN::btnSelectNoneClick);
    connect(ui->btnDelete, &QAbstractButton::clicked, this, &FramePlaybackWindowLIN::btnDeleteCurrSeq);
    ...
    connect(ui->listID, &QListWidget::itemChanged, this, &FramePlaybackWindowLIN::changeIDFiltering);
    connect(ui->btnLoadFile, &QAbstractButton::clicked, this, &FramePlaybackWindowLIN::btnLoadFile);
    connect(ui->btnLoadLive, &QAbstractButton::clicked, this, &FramePlaybackWindowLIN::btnLoadLive);
    connect(ui->tblSequence, &QTableWidget::cellPressed, this, &FramePlaybackWindowLIN::seqTableCellClicked);
    connect(ui->tblSequence, &QTableWidget::cellChanged, this, &FramePlaybackWindowLIN::seqTableCellChanged);
    connect(ui->btnLoadFilters, &QAbstractButton::clicked, this, &FramePlaybackWindowLIN::loadFilters);
    connect(ui->btnSaveFilters, &QAbstractButton::clicked, this, &FramePlaybackWindowLIN::saveFilters);
    connect(ui->cbOriginalTiming, &QCheckBox::toggled, this, &FramePlaybackWindowLIN::useOrigTimingClicked);
    connect(MainWindow::getReference(), SIGNAL(framesUpdated(int)), this, SLOT(updatedFrames(int)));

    connect(&playbackObject, &FramePlaybackObject::EndOfFrameCache, this, &FramePlaybackWindowLIN::EndOfFrameCache);
    connect(&playbackObject, &FramePlaybackObject::statusUpdate, this, &FramePlaybackWindowLIN::getStatusUpdate);
...
FramePlaybackWindowLIN::~FramePlaybackWindowLIN()
{
    delete ui;
}

void FramePlaybackWindowLIN::showEvent(QShowEvent *)
{
    readSettings();
...
void FramePlaybackWindowLIN::closeEvent(QCloseEvent *event)
{
    Q_UNUSED(event);
...
bool FramePlaybackWindowLIN::eventFilter(QObject *obj, QEvent *event)
{
...
void FramePlaybackWindowLIN::readSettings()
{
    QSettings settings;
...
void FramePlaybackWindowLIN::writeSettings()
{
    QSettings settings;
...
void FramePlaybackWindowLIN::contextMenuFilters(QPoint pos)
{
...
void FramePlaybackWindowLIN::saveFilters()
{
    QString filename;
...
void FramePlaybackWindowLIN::loadFilters()
{
    QString filename;
    QFileDialog dialog(this);
    QSettings settings;
...
void FramePlaybackWindowLIN::updatedFrames(int numFrames)
{
    LINFrame thisFrame;
...
void FramePlaybackWindowLIN::refreshIDList()
{
    ...
    disconnect(ui->listID, &QListWidget::itemChanged, this, &FramePlaybackWindowLIN::changeIDFiltering);
...
void FramePlaybackWindowLIN::calculateWhichBus()
{
...
void FramePlaybackWindowLIN::EndOfFrameCache()
{
    if (forward)
    {
...
void FramePlaybackWindowLIN::getStatusUpdate(int frameNum)
{
    currentPosition = frameNum;
    updateFrameLabel();
}

void FramePlaybackWindowLIN::updateFrameLabel()
{
...
void FramePlaybackWindowLIN::seqTableCellClicked(int row, int col)
{
...
void FramePlaybackWindowLIN::seqTableCellChanged(int row,int col)
{
...
void FramePlaybackWindowLIN::fillIDHash(SequenceItem &item)
{
    int id;
...
void FramePlaybackWindowLIN::useOrigTimingClicked()
{
...
void FramePlaybackWindowLIN::btnDeleteCurrSeq()
{
...
void FramePlaybackWindowLIN::btnLoadFile()
{
    QString filename;
    SequenceItem item;
...
void FramePlaybackWindowLIN::btnLoadLive()
{
    SequenceItem item;
    item.filename = "<CAPTURED DATA>";
    item.currentLoopCount = 0;
    item.maxLoops = 1;
    item.data = QVector<LINFrame>(*modelFrames); //create a copy of the current frames from the main view
...
void FramePlaybackWindowLIN::btnBackOneClick()
{
...
void FramePlaybackWindowLIN::btnPauseClick()
{
    isPlaying = false;
    wantPlaying = false;
...
void FramePlaybackWindowLIN::btnReverseClick()
{
    if (!checkNoSeqLoaded()) return;
    forward = false;
    wantPlaying = true;
...
void FramePlaybackWindowLIN::btnStopClick()
{
    isPlaying = false;
    wantPlaying = false;
    haveIncomingTraffic = false;
...
void FramePlaybackWindowLIN::btnPlayClick()
{
    if (!checkNoSeqLoaded()) return;
    forward = true;
    wantPlaying = true;
...
void FramePlaybackWindowLIN::btnFwdOneClick()
{
    if (!checkNoSeqLoaded()) return;
    forward = true;
    isPlaying = false;
...
bool FramePlaybackWindowLIN::checkNoSeqLoaded()
{
    if (seqItems.count() == 0)
    {
...
void FramePlaybackWindowLIN::changePlaybackSpeed(int newSpeed)
{
    playbackObject.setPlaybackInterval(newSpeed);
}

void FramePlaybackWindowLIN::changeBurstRate(int burst)
{
    playbackObject.setPlaybackBurst(burst);
}

void FramePlaybackWindowLIN::changeLooping(bool check)
{
    Q_UNUSED(check);
}

void FramePlaybackWindowLIN::changeSendingBus(int newIdx)
{
    Q_UNUSED(newIdx);
    calculateWhichBus();
}

void FramePlaybackWindowLIN::changeIDFiltering(QListWidgetItem *item)
{
...
void FramePlaybackWindowLIN::btnSelectAllClick()
{
...
void FramePlaybackWindowLIN::btnSelectNoneClick()
{
    for (int i = 0; i < ui->listID->count(); i++)
    {
        QListWidgetItem *item = ui->listID->item(i);
