...
{
    QSettings settings;
    dbHandler * dbHandler = dbHandler::getReference();
    ...
    if (settings.value("Main/FilterLabeling", false).toBool())
    {
        ...
        db_MESSAGE *msg = dbHandler->findMessageForFilter(id,&matchingCriteria);
        if (msg != nullptr)
        {
            ...
            QString tooltip;
            // GMLAN support removed: no longer check matchingCriteria
