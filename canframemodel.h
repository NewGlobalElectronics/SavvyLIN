...
class LINFrameModel: public QAbstractTableModel
{
    Q_OBJECT

public:
    LINFrameModel(QObject *parent = 0);
    virtual ~LINFrameModel();
    ...
    void setIgnoredbColors(bool mode);
    ...
    void insertFrames(const QVector<LINFrame> &newFrames);
    void sortByColumn(int column);
    int getIndexFromTimeID(unsigned int ID, double timestamp);
    const QVector<LINFrame> *getListReference() const; //thou shalt not modify these frames externally!
    const QVector<LINFrame> *getFilteredListReference() const; //Thus saith the Lord, NO.
    const QMap<int, bool> *getFiltersReference() const; //this neither
    const QMap<int, bool> *getBusFiltersReference() const; //this neither

public slots:
    void addFrame(const LINFrame&, bool);
    void addFrames(const CANConnection*, const QVector<LINFrame>&);

signals:
    void updatedFiltersList();

private:
    void qSortCANFrameAsc(QVector<LINFrame>* frames, Column column, int lowerBound, int upperBound);
    void qSortCANFrameDesc(QVector<LINFrame>* frames, Column column, int lowerBound, int upperBound);
    uint64_t getCANFrameVal(QVector<LINFrame> *frames, int row, Column col);
    bool any_filters_are_configured(void);
    bool any_busfilters_are_configured(void);

    QVector<LINFrame> frames;
    QVector<LINFrame> filteredFrames;
    QMap<int, bool> filters;
    QMap<int, bool> busFilters;
    dbHandler *dbHandler;
    QMutex mutex;
    bool interpretFrames; //should we use the dbHandler?
