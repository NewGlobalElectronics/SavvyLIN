...
db Settings
=============
* "Label db Messages by default" - Normally the filter section of the main screen just shows the frame IDs. If you label db signals you will actually see the name of the db message next to the ID.

* "Ignore db message colors" - It's possible to make db messages have rainbow colors. Each message can have a different background and foreground color. Sometimes they can get set to some really silly values. To save your sanity you have the option to completely ignore all colors set in the db file and just use the default colors for everything.
