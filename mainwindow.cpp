...
Allow scripts to read/write signals from db files
allow scripts to load db files in support of the script - maybe the graphing system too.
*/
    ...
    ui(new Ui::MainWindow)
{
    ...
    this->setWindowTitle("SavvyLIN ");

    model = new LINFrameModel(this); // set parent to mainwindow to prevent canframemodel to change thread (might be done by setModel but just in case)
    ...
    settingsDialog = new MainSettingsDialogLIN(); //instantiate the settings dialog so it can initialize settings if this is the first run or the config file was deleted.
   ...
   // connect(ui->actionFlow_View, &QAction::triggered, this, &MainWindow::showFlowViewWindow);
    ...
    //connect(ui->actiondb_Comparison, &QAction::triggered, this, &MainWindow::showdbComparisonWindow);
    ...
    connect(ui->actiondb_File_Manager, &QAction::triggered, this, &MainWindow::showdbFileWindow);
    ...
    connect(ui->actionLDF_Viewer, &QAction::triggered, this, &MainWindow::showLDFViewer);
   ...
   // connect(ui->actionCAN_Bridge, &QAction::triggered, this, &MainWindow::showCANBridgeWindow);
    ...
    connect(model, &LINFrameModel::updatedFiltersList, this, &MainWindow::updateFilterList);
    connect(CANConManager::getInstance(), &CANConManager::framesReceived, model, &LINFrameModel::addFrames);
    ...
    connect(ui->actionLDF_File_Manager, &QAction::triggered, this, &MainWindow::showLDFFileWindow);
    ...
    updateFileStatus();
    //lbStatusDatabase.setText(tr("No db database loaded"));
    ...
    connectionWindow = new ConnectionWindowLIN();
...
void MainWindow::showLDFFileWindow()
{
    qWarning()<<"showLDFFileWindow called";
    qDebug()<<"showLDFFileWindow called";
    if (!ldfEditor)
    {
        ldfEditor = new LDFEditor("");
    }
    ldfEditor->show();
}
...
{
    foreach (GraphingWindowLIN *win, graphWindows)
    {
        killWindow(win);
    }
    ...
    killWindow(connectionWindow);
    if (ldfViewer) { ldfViewer->close(); delete ldfViewer; ldfViewer = nullptr; }
}
...
void MainWindow::readUpdateableSettings()
{
    ...
    ignoredbColors = settings.value("Main/IgnoredbColors", false).toBool();
    model->setIgnoredbColors(ignoredbColors);
...
void MainWindow::gridDoubleClicked(QModelIndex idx)
{
    ...
    LINFrame frame = model->getListReference()->at(idx.row());
...
void MainWindow::setupSendToLatestGraphWindow()
{
    ...
    db_MESSAGE *msg = dbHandler->findMessage(param.ID);
    if(msg)
    {
        DB_SIGNAL *sig = msg->sigHandler->findSignalByName(signalName);
        if(sig)
        {
        ...
        {
            QMessageBox msgbox;
            QString boxmsg = "Cannot find ID 0x" + QStringLiteral("%1").arg(param.ID, 3, 16, QLatin1Char('0')) + " in db message " + msg->name + ". Not adding graph.";
    ...
    {
        QMessageBox msgbox;
        QString boxmsg = "Cannot find ID 0x" + QStringLiteral("%1").arg(param.ID, 3, 16, QLatin1Char('0')) + " in db file(s). Not adding graph.";
        msgbox.setText(boxmsg);
        msgbox.exec();
    }
}
...
void MainWindow::logReceivedFrame(CANConnection* conn, QVector<LINFrame> frames)
{
    Q_UNUSED(conn);
...
void MainWindow::tickGUIUpdate()
{
    ...
    //{
        ...
        if (continuousLogging)
        {
//            const QVector<LINFrame> *modelFrames = model->getListReference();
...
void MainWindow::addFrameToDisplay(LINFrame &frame, bool autoRefresh = false)
{
...
void MainWindow::saveDecodedTextFileAsColumns(QString filename)
{
    ...
    const QVector<LINFrame> *frames = model->getFilteredListReference();
...
*/
    ...
    for (int c = 0; c < frames->count(); c++)
    {
        ...
        {
            db_MESSAGE *msg = dbHandler->findMessage(*frame);
            if (msg != nullptr)
            {
                bool found = false;
...
LINFrameModel* MainWindow::getCANFrameModel()
{
    return model;
}
...
void MainWindow::showSettingsDialog()
{
    if (!settingsDialog)
    {
        settingsDialog = new MainSettingsDialogLIN();
...
*/
    lastGraphingWindow = new GraphingWindowLIN(model->getListReference());
...
void MainWindow::showTemporalGraphWindow()
{
    ...
    if (!temporalGraphWindow)
    {
        ...
        temporalGraphWindow = new TemporalGraphWindowLIN(frames);
    }
...
void MainWindow::showFrameDataAnalysis()
{
    ...
    {
        if (!useFiltered)
            frameInfoWindow = new FrameInfoWindowLIN(model->getListReference());
        else
            frameInfoWindow = new FrameInfoWindowLIN(model->getFilteredListReference());
    }
...
void MainWindow::showISOInterpreterWindow()
{
    ...
    {
        if (!useFiltered)
            isoWindow = new ISOTP_InterpreterWindowLIN(model->getListReference());
        else
            isoWindow = new ISOTP_InterpreterWindowLIN(model->getFilteredListReference());
    }
    isoWindow->show();
}
...
{
    if (!snifferWindow)
        snifferWindow = new SnifferWindowLIN(this);
    snifferWindow->show();
}
...
void MainWindow::showBisectWindow()
{
    if (!bisectWindow)
    {
        bisectWindow = new BisectWindowLIN(model->getListReference());
    }
    bisectWindow->show();
}
...
void MainWindow::showCANBridgeWindow()
{
    if (!canBridgeWindow)
    {
        canBridgeWindow = new LINBridgeWindow(model->getListReference());
    }
...
void MainWindow::showFrameSenderWindow()
{
    ...
    {
        if (!useFiltered)
            frameSenderWindow = new FrameSenderWindowLIN(model->getListReference());
        else
            frameSenderWindow = new FrameSenderWindowLIN(model->getFilteredListReference());
    }
...
void MainWindow::showPlaybackWindow()
{
    ...
    {
        if (!useFiltered)
            playbackWindow = new FramePlaybackWindowLIN(model->getListReference());
        else
            playbackWindow = new FramePlaybackWindowLIN(model->getFilteredListReference());
    }
    playbackWindow->show();
}
...
void MainWindow::showFirmwareUploaderWindow()
{
    ...
    {
        firmwareUploaderWindow = new FirmwareUploaderWindowLIN(model->getListReference());
    }
...
void MainWindow::showComparisonWindow()
{
    if (!comparatorWindow)
    {
        comparatorWindow = new FileComparatorWindowLIN();
    }
    comparatorWindow->show();
}


// void MainWindow::showdbComparisonWindow()
// {
//     if (!dbComparatorWindow)
//     {
//         dbComparatorWindow = new ::dbComparatorWindow();  // Notez le :: devant
//     }
//     dbComparatorWindow->show();
// }
void MainWindow::showSingleMultiWindow()
{
    if (!discreteStateWindow)
    {
        discreteStateWindow = new DiscreteStateWindowLIN(model->getListReference());
    }
...
void MainWindow::showFuzzingWindow()
{
    if (!fuzzingWindow)
    {
        fuzzingWindow = new FuzzingWindowLIN(model->getListReference());
    }
    fuzzingWindow->show();
}
...
void MainWindow::showMCConfigWindow()
{
    ...
    {
        motorctrlConfigWindow = new MotorControllerConfigWindowLIN(model->getListReference());
        //connect(motorctrlConfigWindow, SIGNAL(sendCANFrame(const LINFrame*,int)), worker, SLOT(sendFrame(const LINFrame*,int)));
        //connect(motorctrlConfigWindow, SIGNAL(sendFrameBatch(const QList<LINFrame>*)), worker, SLOT(sendFrameBatch(const QList<LINFrame>*)));
    }
...
void MainWindow::showUDSScanWindow()
{
    if (!udsScanWindow)
    {
        udsScanWindow = new UDSScanWindowLIN(model->getListReference());
    }
    udsScanWindow->show();
}
...
void MainWindow::showScriptingWindow()
{
    if (!scriptingWindow)
    {
        scriptingWindow = new ScriptingWindowLIN(model->getListReference());
    }
...
void MainWindow::showRangeWindow()
{
    if (!rangeWindow)
    {
        rangeWindow = new RangeStateWindowLIN(model->getListReference());
    }
    rangeWindow->show();
}
...
void MainWindow::showFlowViewWindow()
{
    ...
    {
        if (!useFiltered)
            flowViewWindow = new FlowViewWindowLIN(model->getListReference());
        else
            flowViewWindow = new FlowViewWindowLIN(model->getFilteredListReference());
...
void MainWindow::dbSettingsUpdated()
    {
    updateFilterList();
    model->sendRefresh();
    }

void MainWindow::showdbFileWindow()
{
    if (!dbFileWindow)
    {
        dbFileWindow = new dbLoadSaveWindow(model->getListReference());
        connect(dbFileWindow, &dbLoadSaveWindow::updateddbSettings, this, &MainWindow::dbSettingsUpdated);
    }
    dbFileWindow->show();
}
void MainWindow::showLDFViewer()
{
    qWarning()<<"LDF Viewer functions called";

    if (!ldfViewer)
    {
        ldfViewer = new LDFViewer(this, "");
    }
    ldfViewer->show();
}
void MainWindow::showSignalViewer()
{
    ...
    {
        if (!useFiltered)
            signalViewerWindow = new SignalViewerWindowLIN(model->getListReference());
        else
            signalViewerWindow = new SignalViewerWindowLIN(model->getFilteredListReference());
    }
