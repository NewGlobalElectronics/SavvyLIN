...
MotorControllerConfigWindowLIN::MotorControllerConfigWindowLIN(const QVector<LINFrame> *frames, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::MotorControllerConfigWindowLIN)
{
    ui->setupUi(this);
...
MotorControllerConfigWindowLIN::~MotorControllerConfigWindowLIN()
{
    delete ui;
}

void MotorControllerConfigWindowLIN::updatedFrames(int numFrames)
{
    LINFrame thisFrame;
    uint32_t id;
                ...
                if ((char)thisFrame.payload()[2] == 0)
                {
                    ...
                    for (int i = 0; i < params.length(); i++)
                    {
                        if (params[i].paramID == paramID)
                        {
                            ...
                            if (params[i].paramType == PARAM_ASCII) item = new QTableWidgetItem(); //QString::fromUtf8((char *)params[i].value, 2));
                            if (params[i].paramType == PARAM_DEC) item = new QTableWidgetItem(Utility::formatHexNum(params[i].value));
                            if (params[i].paramType == PARAM_DEC)
                            {
...
void MotorControllerConfigWindowLIN::refreshData()
{
    doingRequest = true;
    transmitStep = 0;
    timer.start();
...
void MotorControllerConfigWindowLIN::loadFile()
{
    QString filename;
    QFileDialog dialog;
    QFile *inFile;
    ...
    if (dialog.exec() == QDialog::Accepted)
    {
        ...
        while (!inFile->atEnd())
        {
            ...
            if (!line.startsWith("#") && line.length() > 10)
            {
                ...
                //Serial_Number_EEPROM                     , 0x0113, uint, PARAM_DEC, 16, 6, spr, spr, spr
                ...
                param.paramType = PARAM_DEC;
                if (typeStr.compare("PARAM_DEC") == 0) param.paramType = PARAM_DEC;
                if (typeStr.compare("PARAM_ASCII") == 0) param.paramType = PARAM_ASCII;
...
void MotorControllerConfigWindowLIN::saveData()
{
    doingRequest = false;
    transmitStep = 0;
    timer.start();
...
void MotorControllerConfigWindowLIN::timerTick()
{
    ...
    else //save current value of all data items
    {
        ...
        if (params[transmitStep].paramType != PARAM_DEC)
