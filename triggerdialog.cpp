    ...
    ui(new Ui::TriggerDialog)
{
    ...
    /*
      getting the messages possible is complicated by the fact that multiple db files
    ...
    */
    ui->cmMsgID->clear();
    QStringList entries;
    int numFiles = dbHandler->getFileCount();
    for (int i = 0; i < numFiles; i++)
    {
        ...
        {
            db_MESSAGE *msg = file->messageHandler->findMsgByIdx(j);
...
void TriggerDialog::regenerateCurrentListItem()
{
    ...
    if (ui->cbMsgID->isChecked() && ui->cmMsgID->currentText().length() > 0)
    {
        ...
        if (trig.ID != lastMsgID)
        {
            ...
            //if possible.
            db_MESSAGE *msg = dbHandler->findMessage(trig.ID);
            if (msg)
            {
...
void TriggerDialog::FillDetails()
{
    ...
    if (triggers[selectedRow].triggerMask & TriggerMask::TRG_ID)
    {
        ...
        db_MESSAGE *msg = dbHandler->findMessage(triggers[selectedRow].ID);
        if (msg)
        {
